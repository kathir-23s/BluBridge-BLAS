...
// ============================================================
// Bgemm Backward TN V18 — dB = dX^T dY (C = A^T * B)
// ============================================================
    ...
    int batchCount)
{
    ...
    auto load_regA = [&](uint32_t reg[4], int ki, int mi, int st) {
        int m = wy * 64 + mi * 16 + ((lane / 8) % 2) * 8 + (lane % 8);
        int k = ki + (lane / 16) * 8;
    ...
    auto load_regB = [&](uint32_t reg[2], int ki, int ni, int st) {
        int n = wx * 64 + ni * 16 + ((lane / 8) % 2) * 8 + (lane % 8);
        int k = ki + (lane / 16) * 8;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            int r = wy * 64 + i * 16 + rt;
            int c = wx * 64 + j * 8 + ct;
            s_mem[r * BN + c]     = __float2bfloat16(acc[i][j][0] * alpha_f);
            s_mem[r * BN + c + 1] = __float2bfloat16(acc[i][j][1] * alpha_f);
            s_mem[(r+8) * BN + c]     = __float2bfloat16(acc[i][j][2] * alpha_f);
            s_mem[(r+8) * BN + c + 1] = __float2bfloat16(acc[i][j][3] * alpha_f);
        }
    }
    __syncthreads();
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (gr < M && gc < N) {
            __nv_bfloat16* d_ptr = C_batch + (long long)gr * ldc + gc;
            __nv_bfloat16* s_ptr = &s_mem[r * BN + c];
            #pragma unroll
            for (int l = 0; l < 8; l++) {
                if (gc + l < N) {
                    d_ptr[l] = __float2bfloat16(__bfloat162float(s_ptr[l]) + beta_f * __bfloat162float(d_ptr[l]));
                }
            }
        }
    }
...
extern "C" void mycublasBgemmStridedBatched_tn(
    ...
    const __nv_bfloat16 beta, __nv_bfloat16* d_C, int ldc, long long int strideC, 
    int batchCount)
{
    cudaStream_t stream = (handle != nullptr) ? handle->stream : 0;
    static const size_t smem_bytes = STAGES * STG_SZ * sizeof(__nv_bfloat16);
    ...
    if (aligned) {
        set_attr((const void*)bgemm_tn_backward_kernel<true>, smem_bytes);
        bgemm_tn_backward_kernel<true><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,beta,d_C,ldc,strideC,batchCount);
    } else {
        set_attr((const void*)bgemm_tn_backward_kernel<false>, smem_bytes);
        bgemm_tn_backward_kernel<false><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,beta,d_C,ldc,strideC,batchCount);
    }
}
