...
sgemm_tn_128x128_sm89_2_k(int M, int N, int K, float alpha, const float* A, int lda, long long strideA, const float* B, int ldb, long long strideB, float beta, float* C, int ldc, long long strideC, int batchCount, int splitK) {
    ...
    for (int mi = 0; mi < MMA_M; mi++) {
        ...
        for (int ni = 0; ni < MMA_N; ni++) {
            ...
            if (splitK > 1) {
                atomicAdd(d, alpha * acc[mi][ni][0]);
                if (c0 + 1 < N) atomicAdd(d + 1, alpha * acc[mi][ni][1]);
                if (r8 < M) {
                    float* d8 = C_ptr + (long long)r8 * ldc + c0;
                    atomicAdd(d8, alpha * acc[mi][ni][2]);
                    if (c0 + 1 < N) atomicAdd(d8 + 1, alpha * acc[mi][ni][3]);
                }
            } else {
...
extern "C" void launch_sgemm_tn_128x128_sm89_2(int M, int N, int K, float alpha, const float* A, int lda, long long strideA, const float* B, int ldb, long long strideB, float beta, float* C, int ldc, long long strideC, int batchCount, int splitK, cudaStream_t stream) {
    using namespace tn128x128;
    static constexpr int SMEM = STAGES * STAGE_SIZE * sizeof(float);
    static bool done = false; if (!done) { cudaFuncSetAttribute(sgemm_tn_128x128_sm89_2_k, cudaFuncAttributeMaxDynamicSharedMemorySize, SMEM); done = true; }
    sgemm_tn_128x128_sm89_2_k<<<dim3((N + BN - 1) / BN, (M + BM - 1) / BM, batchCount * splitK), THREADS, SMEM, stream>>>(M, N, K, alpha, A, lda, strideA, B, ldb, strideB, beta, C, ldc, strideC, batchCount, splitK);
}
