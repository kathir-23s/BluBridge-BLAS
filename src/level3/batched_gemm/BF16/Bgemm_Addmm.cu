...
// ============================================================
// Bgemm Addmm V34 — Fused Matmul + Bias (BF16)
//
// Highlights:
//   1. Derived from Sgemm_addmm_v34 (Fused Bias + Vectorized Epilogue)
//   2. Uses mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32
    ...
    int batchCount)
{
    ...
    auto load_to_stage = [&](int stage, int k_curr) {
        ...
        for (int i = 0; i < 4; i++) {
            int r = tid; 
            int c = i * 8;
            const __nv_bfloat16* g_ptr = A_batch + (long long)(by * BM + r) * lda + (k_curr + c);
        ...
        for (int i = 0; i < 4; i++) {
            int r = tid / 4; 
            int c = (tid % 4) * 8 + i * 32; 
            const __nv_bfloat16* g_ptr = B_batch + (long long)(k_curr + r) * ldb + (bx * BN + c);
    ...
    for (int j = 0; j < 8; j++) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (t % 2 == 0) {
                ...
                if (bias) {
                    if (bias_numel == 1) { float b = __bfloat162float(bias[0]); bv0 = bv8 = {b,b,b,b}; }
                    else if (bias_numel == N) {
                         if (c < N) {
                             if (c + 7 < N && (((size_t)(&bias[c]) & 15) == 0)) {
                                 uint4 b_vec = *(uint4*)&bias[c];
                                 __nv_bfloat16* bh = (__nv_bfloat16*)&b_vec;
                                 bv0 = {__bfloat162float(bh[0]), __bfloat162float(bh[1]), __bfloat162float(bh[2]), __bfloat162float(bh[3])};
                                 bv8 = bv0;
                             } else {
                                 bv0.x = __bfloat162float(bias[c]); 
                                 if(c+1<N) bv0.y = __bfloat162float(bias[c+1]); 
                                 if(c+2<N) bv0.z = __bfloat162float(bias[c+2]); 
                                 if(c+3<N) bv0.w = __bfloat162float(bias[c+3]);
                                 bv8 = bv0;
                             }
                         }
                         ...
                         if (r0 < M && c < N) {
                             bv0.x = __bfloat162float(bias[idx0]); if(c+1<N) bv0.y = __bfloat162float(bias[idx0+1]); if(c+2<N) bv0.z = __bfloat162float(bias[idx0+2]); if(c+3<N) bv0.w = __bfloat162float(bias[idx0+3]);
                         }
                         if (r8 < M && c < N) {
                             bv8.x = __bfloat162float(bias[idx8]); if(c+1<N) bv8.y = __bfloat162float(bias[idx8+1]); if(c+2<N) bv8.z = __bfloat162float(bias[idx8+2]); if(c+3<N) bv8.w = __bfloat162float(bias[idx8+3]);
                         }
                    }
                }
                ...
                auto sb4 = [&](int r, int cl, float4 v) {
                    if (r < M && cl < N) {
                        ...
                        if(cl+1<N) p[1] = __float2bfloat16(v.y);
                        if(cl+2<N) p[2] = __float2bfloat16(v.z);
                        if(cl+3<N) p[3] = __float2bfloat16(v.w);
                    }
                };
                sb4(r0, c, f4_r0); sb4(r8, c, f4_r8);
            }
        }
    }
}

extern "C" void mycublasBgemmAddmm_v34(
    ...
    const __nv_bfloat16* d_bias, int64_t bias_numel,
    __nv_bfloat16* d_C, int ldc, long long int strideC, int batchCount)
{
    cudaStream_t stream = (handle != nullptr) ? handle->stream : 0;
    static const size_t smem_bytes = STAGES * STAGE_SIZE * sizeof(__nv_bfloat16);
    
    static bool attr_set = false;
    if (!attr_set) {
        cudaFuncSetAttribute(bgemm_addmm_v34_kernel<true>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem_bytes);
        cudaFuncSetAttribute(bgemm_addmm_v34_kernel<false>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem_bytes);
        attr_set = true;
    }

    dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM, batchCount);
    bgemm_addmm_v34_kernel<true><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,beta,d_bias,bias_numel,d_C,ldc,strideC,batchCount);
}
