...
// ============================================================
// Optimized Backward SGEMM NT Kernel V18
// dA = dY dX^T (A: [M, K], B: [N, K], C: [M, N])
// Mapping: C = A * B^T
    ...
    int batchCount)
{
    ...
    #pragma unroll
    for (int i = 0; i < 4; i++) {
        int r = (tid / 4) + i * 32, c = (tid % 4) * 4;
        gA_ptr[i] = A + (long long)batch * strideA + (long long)(by * BM + r) * lda + (k_start + c);
        gB_ptr[i] = B + (long long)batch * strideB + (long long)(bx * BN + r) * ldb + (k_start + c);
    }
    ...
    auto load_to_stage = [&](int stage, int ko) {
        ...
        #pragma unroll
        for (int i = 0; i < 4; i++) {
            int r = (tid / 4) + i * 32, c = (tid % 4) * 4, sc = c ^ ((r & 3) << 2);
            uint32_t sm_a = __cvta_generic_to_shared(&As[r * BK + sc]);
            int gr = by * BM + r, gc = ko + c;
            if constexpr (IsAligned) {
                ...
                if (gr < M && gc < K) {
                    val.x = gA_ptr[i][0];
                    if (gc + 1 < K) val.y = gA_ptr[i][1];
                    if (gc + 2 < K) val.z = gA_ptr[i][2];
                    if (gc + 3 < K) val.w = gA_ptr[i][3];
                }
        ...
        #pragma unroll
        for (int i = 0; i < 4; i++) {
            int r = (tid / 4) + i * 32, c = (tid % 4) * 4, sc = c ^ ((r & 6) << 1);
            uint32_t sm_b = __cvta_generic_to_shared(&Bs[r * BK + sc]);
            int gr = bx * BN + r, gc = ko + c;
            if constexpr (IsAligned) {
                ...
                if (gr < N && gc < K) {
                    val.x = gB_ptr[i][0];
                    if (gc + 1 < K) val.y = gB_ptr[i][1];
                    if (gc + 2 < K) val.z = gB_ptr[i][2];
                    if (gc + 3 < K) val.w = gB_ptr[i][3];
                }
                *(float4*)&Bs[r * BK + sc] = val;
            }
            gB_ptr[i] += BK;
        }
    };
    ...
    auto load_frA = [&](uint32_t reg[4], int ks, int mi, int st) {
        ...
        reg[1] = ga(rbaseA[mi] + 8 * BK, ks + t_sh);
        reg[2] = ga(rbaseA[mi], ks + t_sh + 4); 
        reg[3] = ga(rbaseA[mi] + 8 * BK, ks + t_sh + 4);
    };

    int rbaseB[8];
    #pragma unroll
    for (int j = 0; j < 8; j++) rbaseB[j] = (wx * 64 + j * 8 + g_sh) * BK;

    auto load_frB = [&](uint32_t reg[2], int ks, int ni, int st) {
        float* Bs = smem + st * STAGE_SIZE + AS_SIZE;
        const int row = rbaseB[ni], mask = ((row / BK) & 6) << 1;
...
__global__ void sgemm_nt_scale_kernel_SM86(float* C, float beta, int M, int N, int ldc, long long strideC, int batchCount) {
    ...
    int batchCount)
{
    ...
    // Tier 1: GEMV
    if (M <= 16 || N <= 16) {
        mycublasSgemv_nt_SM86(handle, M, N, K, alpha,
    ...
    if (gx * gy < sm_count) {
            ...
            mycublasSgemmStridedBatched_nt_tile64_SM86(handle, M, N, K, alpha,
                                                      d_A, lda, strideA, d_B, ldb, strideB,
                                                      beta, d_C, ldc, strideC, batchCount);
        return;
    }
    ...
    // NT uses sk=16 max. Higher sk causes L2 atomic contention on small output tiles.
    int sk = 1; if (gx * gy < sm_count * 2 && K >= 1024) { sk = 4; if (gx * gy < sm_count / 2) sk = 8; if (gx * gy < sm_count / 8) sk = 16; }
    if (sk > 1) { dim3 s_grid((N+31)/32, (M+31)/32, batchCount); sgemm_nt_scale_kernel_SM86<<<s_grid, dim3(32,32), 0, stream>>>(d_C, beta, M, N, ldc, strideC, batchCount); }

    dim3 grid(gx, gy, batchCount * sk);
    if (aligned) {
        if (sk == 1) { set_limit((const void*)sgemm_nt_backward_kernel_SM86<true, 1>, smem_bytes); sgemm_nt_backward_kernel_SM86<true, 1><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,beta,d_C,ldc,strideC,batchCount); }
        else if (sk == 4) { set_limit((const void*)sgemm_nt_backward_kernel_SM86<true, 4>, smem_bytes); sgemm_nt_backward_kernel_SM86<true, 4><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,0.f,d_C,ldc,strideC,batchCount); }
        else if (sk == 8) { set_limit((const void*)sgemm_nt_backward_kernel_SM86<true, 8>, smem_bytes); sgemm_nt_backward_kernel_SM86<true, 8><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,0.f,d_C,ldc,strideC,batchCount); }
        else { set_limit((const void*)sgemm_nt_backward_kernel_SM86<true, 16>, smem_bytes); sgemm_nt_backward_kernel_SM86<true, 16><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,0.f,d_C,ldc,strideC,batchCount); }
    } else {
        if (sk == 1) { set_limit((const void*)sgemm_nt_backward_kernel_SM86<false, 1>, smem_bytes); sgemm_nt_backward_kernel_SM86<false, 1><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,beta,d_C,ldc,strideC,batchCount); }
        else if (sk == 4) { set_limit((const void*)sgemm_nt_backward_kernel_SM86<false, 4>, smem_bytes); sgemm_nt_backward_kernel_SM86<false, 4><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,0.f,d_C,ldc,strideC,batchCount); }
        else if (sk == 8) { set_limit((const void*)sgemm_nt_backward_kernel_SM86<false, 8>, smem_bytes); sgemm_nt_backward_kernel_SM86<false, 8><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,0.f,d_C,ldc,strideC,batchCount); }
        else { set_limit((const void*)sgemm_nt_backward_kernel_SM86<false, 16>, smem_bytes); sgemm_nt_backward_kernel_SM86<false, 16><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,0.f,d_C,ldc,strideC,batchCount); }
    }
}