...
enum class HgemmLayout { NT, TN, NN };

template <typename Config, bool IsAligned, int SplitK, HgemmLayout Layout>
__global__ void __launch_bounds__(Config::THREADS, 1)
hgemm_backward_template_kernel(
    int M, int N, int K,
    __half alpha,
    ...
    int batchCount)
{
    ...
    extern __shared__ __align__(16) __half h_smem[];
    ...
    const __half* gA_ptr[Layout == HgemmLayout::TN ? Config::TN_LOAD_ITERS_A : Config::NT_LOAD_ITERS_A];
    const __half* gB_ptr[Layout == HgemmLayout::NT ? Config::NT_LOAD_ITERS_B : Config::TN_LOAD_ITERS_B];

    if constexpr (Layout == HgemmLayout::TN) {
        #pragma unroll
    ...
    if constexpr (Layout == HgemmLayout::NT) {
        #pragma unroll
    ...
    auto load_to_stage = [&](int stage, int ko) {
        __half* As = h_smem + stage * Config::STAGE_SIZE; __half* Bs = As + Config::AS_SIZE;
        if constexpr (Layout == HgemmLayout::TN) {
            #pragma unroll
            for (int i = 0; i < Config::TN_LOAD_ITERS_A; i++) {
    ...
    auto load_frA = [&](uint32_t reg[4], int ks, int mi, int st) {
        __half* As = h_smem + st * Config::STAGE_SIZE;
        if constexpr (Layout == HgemmLayout::TN) {
    ...
    auto load_frB = [&](uint32_t reg[2], int ks, int ni, int st) {
        __half* Bs = h_smem + st * Config::STAGE_SIZE + Config::AS_SIZE;
        if constexpr (Layout == HgemmLayout::NT) {
    ...
    for (int k = k_start; k < k_end; k += Config::BK) {
        ...
        for (int ks = 0; ks < Config::BK; ks += 16) {
            ...
            for (int i = 0; i < Config::MMA_M; i++) {
                ...
                #pragma unroll
                for (int j = 0; j < Config::MMA_N; j++) MMA_M16N8K16_F32(acc[i][j][0], acc[i][j][1], acc[i][j][2], acc[i][j][3], frA[p][i][0], frA[p][i][1], frA[p][i][2], frA[p][i][3], frB[p][j][0], frB[p][j][1]);
            }
        }
    }
    asm volatile("cp.async.wait_group 0;\n"); __syncthreads();
    __half* s_out = h_smem; const int rt = lane / 4, ct = (lane % 4) * 2; const float alpha_f = __half2float(alpha);
    #pragma unroll
    for (int i = 0; i < Config::MMA_M; i++) {
        #pragma unroll
        for (int j = 0; j < Config::MMA_N; j++) {
            ...
            *(__half2*)&s_out[swizzled_r * Config::BN + c] = {__float2half(acc[i][j][0] * alpha_f), __float2half(acc[i][j][1] * alpha_f)};
            *(__half2*)&s_out[(swizzled_r ^ 8) * Config::BN + c] = {__float2half(acc[i][j][2] * alpha_f), __float2half(acc[i][j][3] * alpha_f)};
        }
    }
    __syncthreads();
    ...
    for (int i = 0; i < (Config::BM * Config::BN) / (Config::THREADS * 8); i++) {
        ...
        if (gr < M && gc < N) {
            ...
            if constexpr (SplitK > 1) {
                ...
                for (int l = 0; l < 4; l++) {
                    if (gc + l * 2 + 1 < N) {
                        atomicAdd((__half2*)((__half*)C + (long long)batch * strideC + (long long)gr * ldc + gc + l * 2), h2v[l]);
                    } else {
                        atomicAdd((__half*)C + (long long)batch * strideC + (long long)gr * ldc + gc + l * 2, h2v[l].x);
                    }
                }
            } else {
                if (beta != __float2half(0.0f)) {
                    int4 old = *(int4*)&C[(long long)batch * strideC + (long long)gr * ldc + gc];
                    __half2* h2v = (__half2*)&vals, * h2o = (__half2*)&old;
                    #pragma unroll
...
__global__ void hgemm_scale_template_kernel(__half* C, __half beta, int M, int N, int ldc, long long strideC, int batchCount, const __half* bias = nullptr, int64_t bias_numel = 0) {
    int r = blockIdx.y * blockDim.y + threadIdx.y, c = blockIdx.x * blockDim.x + threadIdx.x, b = blockIdx.z;
    if (r < M && c < N && b < batchCount) {
        __half* dst = &C[b * strideC + (long long)r * ldc + c];
        float val = (beta == __float2half(0.f)) ? 0.f : (__half2float(*dst) * __half2float(beta));
        if (bias) {
            if (bias_numel == N) val += __half2float(bias[c]);
            else if (bias_numel == M) val += __half2float(bias[r]);
            else if (bias_numel == 1) val += __half2float(bias[0]);
            else if (bias_numel == (int64_t)M*N) val += __half2float(bias[(long long)r * N + c]);
        }
        *dst = __float2half(val);
    }
}
