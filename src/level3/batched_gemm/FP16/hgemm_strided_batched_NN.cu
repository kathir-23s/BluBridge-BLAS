...
// V47: Peak Performance Interleaved MMA (128x128x32 Tile, 4 Warps, 3-Stage)
...
hgemm_NN_kernel(
    ...
    __half beta,
    __half* __restrict__ C_ptr, int ldc, long long int strideC,
    int batchCount)
{
    ...
    const __half* A_batch = A + (long long)batch_idx * strideA;
    const __half* B_batch = B + (long long)batch_idx * strideB;
    __half* C_batch = C_ptr + (long long)batch_idx * strideC;
    ...
    auto load_to_stage = [&](int stage, int k_off) {
        ...
        // Each thread loads one int4 (8 elements). Total 1024 elements per iteration.
        // We need 4 iterations to load 4096 elements.
        #pragma unroll
        for (int i = 0; i < 4; i++) {
        ...
        // Coalesced loading: 16 threads per row.
        #pragma unroll
        for (int i = 0; i < 4; i++) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            int r = wy * 64 + i * 16 + rt;
            int c = wx * 64 + j * 8 + ct;
            __half2 h2_01 = {__float2half(acc[i][j][0] * alpha_f), __float2half(acc[i][j][1] * alpha_f)};
            __half2 h2_23 = {__float2half(acc[i][j][2] * alpha_f), __float2half(acc[i][j][3] * alpha_f)};
            *(__half2*)&s_out[r * BN + c] = h2_01;
            *(__half2*)&s_out[(r+8) * BN + c] = h2_23;
        }
    }
    __syncthreads();
    ...
    const __half beta, __half *C, int ldc, long long int strideC, int batchCount)
{
    ...
    size_t smem_size = (STAGES * (BM * BK + BN * BK)) * sizeof(__half); 
    ...
    auto launch = [&](auto tA, auto tB) {
        ...
        if (!smem_set[idx]) {
            cudaFuncSetAttribute(hgemm_NN_kernel<TA, TB>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem_size);
            smem_set[idx] = true;
        }
        dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM, batchCount);
        dim3 block(THREADS_PER_BLOCK);
        hgemm_NN_kernel<TA, TB><<<grid, block, smem_size, stream>>>(
