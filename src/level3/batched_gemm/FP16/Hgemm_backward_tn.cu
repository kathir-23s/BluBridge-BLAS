...
// ============================================================
// Hgemm Backward TN V18 — dB = dX^T dY (C = A^T * B)
// ============================================================
    ...
    int batchCount)
{
    ...
    auto load_tile = [&](int stage, int ko) {
        ...
        for (int i = 0; i < 4; i++) {
            int r = (tid / 16) + i * 8; 
            int c = (tid % 16) * 8;     
            const __half* g_ptr = A_batch + (long long)(ko + r) * lda + (by * BM + c);
        ...
        for (int i = 0; i < 4; i++) {
            int r = (tid / 16) + i * 8; 
            int c = (tid % 16) * 8;     
            const __half* g_ptr = B_batch + (long long)(ko + r) * ldb + (bx * BN + c);
            int swizzled_c = c ^ ((r & 7) << 3); 
...
extern "C" void mycublasHgemmStridedBatched_tn(
    ...
    int batchCount)
{
    ...
    if (aligned) {
        set_attr((const void*)hgemm_tn_backward_kernel<true>, smem_bytes);
        hgemm_tn_backward_kernel<true><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,beta,d_C,ldc,strideC,batchCount);
    } else {
        set_attr((const void*)hgemm_tn_backward_kernel<false>, smem_bytes);
        hgemm_tn_backward_kernel<false><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,beta,d_C,ldc,strideC,batchCount);
    }
}
