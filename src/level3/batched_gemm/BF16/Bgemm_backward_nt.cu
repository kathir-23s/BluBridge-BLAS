...
// ============================================================
// Bgemm Backward NT V18 — dA = dY dX^T (C = A * B^T)
// ============================================================
...
extern "C" void mycublasBgemmStridedBatched_nt(
    ...
    int batchCount)
{
    ...
    if (aligned) {
        set_attr((const void*)bgemm_nt_backward_kernel<true>, smem_bytes);
        bgemm_nt_backward_kernel<true><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,beta,d_C,ldc,strideC,batchCount);
    } else {
        set_attr((const void*)bgemm_nt_backward_kernel<false>, smem_bytes);
        bgemm_nt_backward_kernel<false><<<grid, THREADS, smem_bytes, stream>>>(M,N,K,alpha,d_A,lda,strideA,d_B,ldb,strideB,beta,d_C,ldc,strideC,batchCount);
    }
}
