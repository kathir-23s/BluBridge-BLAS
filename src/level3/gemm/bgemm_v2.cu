...
// BGEMM V2: 128x128x64 Tile, 4 Warps, 3-Stage Pipeline + CTA Swizzle
// Based on BGEMM Strided Batched V5
// =====================================================================
...
bgemm_v2_kernel(
    ...
    __nv_bfloat16 beta,
    __nv_bfloat16* __restrict__ C_ptr, int ldc,
    int grid_x, int grid_y)
{
    ...
    auto load_to_stage = [&](int stage, int k_off) {
        ...
        for (int i = 0; i < 8; i++) {
            int r = tid;
            int c = i * 8;
            const __nv_bfloat16* g_ptr = A + (long long)(by * BM + r) * lda + (k_off + c);
        ...
        for (int i = 0; i < 8; i++) {
            int r = tid / 2;
            int c = (tid % 2) * 64 + i * 8;  
            const __nv_bfloat16* g_ptr = B + (long long)(k_off + r) * ldb + (bx * BN + c);
            int swizzled_c = c ^ ((r & 7) << 3);
    ...
    for (int i = 0; i < 4; i++) {
        #pragma unroll
        for (int j = 0; j < 8; j++) {
            int r = wy * 64 + i * 16 + rt, c = wx * 64 + j * 8 + ct;
            s_out[r * BN + c]         = __float2bfloat16(acc[i][j][0] * alpha_f);
            s_out[r * BN + c + 1]     = __float2bfloat16(acc[i][j][1] * alpha_f);
            s_out[(r+8) * BN + c]     = __float2bfloat16(acc[i][j][2] * alpha_f);
            s_out[(r+8) * BN + c + 1] = __float2bfloat16(acc[i][j][3] * alpha_f);
        }
    }
    __syncthreads();
...
extern "C" void mycublasBgemm_v2(
    ...
    const __nv_bfloat16 beta, __nv_bfloat16 *C, int ldc)
{
    cudaStream_t stream = (handle != nullptr) ? handle->stream : 0;
    static bool smem_set = false;
    size_t smem_size = (size_t)STAGES * (BM * BK + BK * BN) * sizeof(__nv_bfloat16);
    if (!smem_set) {
        cudaFuncSetAttribute(bgemm_v2_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem_size);
        smem_set = true;
    }
    int grid_x = (N + BN - 1) / BN, grid_y = (M + BM - 1) / BM;
    dim3 grid(grid_x, grid_y);
    dim3 block(THREADS_PER_BLOCK);
    bgemm_v2_kernel<<<grid, block, smem_size, stream>>>(M, N, K, alpha, A, lda, B, ldb, beta, C, ldc, grid_x, grid_y);
}
